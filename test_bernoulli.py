...
#funcs = [euler, genocchi]
funcs = [dirichlet_eta, bernoulli, zeta, harmonic, genocchi, euler]
        ...
        for p1 in points:
            for p2 in points:
                try:
                    rep = {x:p1, y:p2}
                    if (
                        expr1.subs(rep).evalf(chop=True).n(4)
                            !=
                        expr2.subs(rep).evalf(chop=True).n(4)
                        #expr1.evalf(subs=rep, chop=True).n(4)
                        #    !=
                        #expr2.evalf(subs=rep, chop=True).n(4)
                        ):
                        breakpoint()
                        print()
                        print(f1, f2, rep)
